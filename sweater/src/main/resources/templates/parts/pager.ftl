<#macro pager url page>
    <#if page.getTotalPages() gt 7 >
        ...
        headPage = (pageNumber > 4)?then([1,-1],[1,2,3])
        tailPage = (pageNumber > totalPages - 3)?then([totalPages - 2, totalPages - 1, totalPages], [-1, totalPages])
        bodyBefore = (pageNumber > 4 && pageNumber < totalPages - 1)?then([pageNumber - 2, pageNumber - 1], [])
        bodyAfter = (pageNumber > 2 && pageNumber < totalPages - 3)?then([pageNumber + 1, pageNumber + 2], [])
        body = headPage + bodyBefore + (pageNumber > 3 && pageNumber < totalPages - 2)?then([pageNumber],[]) + bodyAfter + tailPage
        >
    <#else>
